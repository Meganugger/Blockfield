...
export default function InventoryBar({ slots }) {
  ...
  return (
    <div className="absolute bottom-14 left-1/2 -translate-x-1/2 flex gap-1.5 pointer-events-none">
      {slots.map((item, i) => (
        <div
          key={i}
          className="w-12 h-12 rounded-lg bg-black/45 backdrop-blur-md border border-white/15 flex items-center justify-center relative"
        >
          {item ? (
            <>
              <div
